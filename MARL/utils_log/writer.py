...
from MARL.configs import base_config as cfg
from MARL.utils_log.decorators import check_main_thread, decorate_all
from MARL.utils_log.printing import human_format
...
class EventName(enum.Enum):
    ...
    ETA = "ETA (time)"
    TRAIN_ITEMS_PER_SEC = "Train Items / Sec"
    TEST_ITEMS_PER_SEC = "Test Items / Sec"
    VIS_ITEMS_PER_SEC = "Vis Items / Sec"
...
@check_main_thread
def put_image(name, image: Union[TensorType["H", "W", "C"], any], step: Optional[int]):
...
@check_main_thread
def put_scalar(name: str, scalar: Any, step: Optional[int]):
...
@check_main_thread
def put_dict(name: str, scalar_dict: Dict[str, Any], step: Optional[int]):
...
@check_main_thread
def put_config(name: str, config_dict: Dict[str, Any], step: Optional[int]):
    """Setter function to place a dictionary of scalars into the queue to be written out
...
@check_main_thread
def put_model(name: str, model: any, data_in: any):
    """Setter function to place a model into the queue to be written out

    Args:
        name: name of model
        model: model to write out
        data_in: data to input to the model
    """
    EVENT_STORAGE.append({"name": name, "write_type": EventType.MODEL, "event": model, "step": data_in})
...
@check_main_thread
def setup_event_writer(is_wandb_enabled: bool, is_tensorboard_enabled: bool, entity: str, projectname: str, log_dir: Path) -> None:
    """Initialization of all event writers specified in config
    ...
    if is_wandb_enabled:
        curr_writer = WandbWriter(entity=entity, projectname=projectname, log_dir=log_dir)
        EVENT_WRITERS.append(curr_writer)
...
class Writer:
    ...
    @abstractmethod
    def write_model(self, name: str, model: any, data_in: any) -> None:
...
class WandbWriter(Writer):
    ...
    def __init__(self, entity: str, projectname: str, log_dir: Path):
        wandb.init(entity=entity, project=projectname, dir=str(log_dir), reinit=True)
        CONSOLE.print(f"[bold red]Given Step is not used with Wandb with the current implementation!!!")

    def write_image(self, name: str, image: Union[TensorType["H", "W", "C"], any], step: int) -> None:
        img = image
        if isinstance(image, torch.Tensor):
            img = torch.permute(image, (2, 0, 1))
        wandb.log({name: wandb.Image(img)}, step=step)

    def write_scalar(self, name: str, scalar: Union[float, torch.Tensor], step: Optional[int]) -> None:
        #if step is None:
        wandb.log({name: scalar})
        #else:
...
class TensorboardWriter(Writer):
    ...
    def write_model(self, name: str, model: any, data_in: any) -> None:
        ...
        Args:
            ...
            indata: input data (required to create the model)
        """
        self.tb_writer.add_graph(model, data_in)
