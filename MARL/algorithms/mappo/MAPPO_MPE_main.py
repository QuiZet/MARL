...
class Runner_MAPPO_MPE:
    def __init__(self, args, env_name, number, seed):
        ...
        # Create env
        self.env = make_env_from_pettingzoo(env_name, num_good = 2, num_adversaries=4, num_obstacles=0, render_mode='human') # Discrete action space
        self.env.reset()
        ...
        #print(f'self.args.obs_dim_n_raw:{self.args.obs_dim_n_raw}')
        self.args.obs_dim_n = []
        for box in self.args.obs_dim_n_raw:
            obs_dim = box.shape[0]
            self.args.obs_dim_n.append(obs_dim)
        self.args.obs_dim_n = [max(self.args.obs_dim_n)] * self.args.N # The dimensions of an agent's observation space
        print(f'self.args.obs_dim_n:{self.args.obs_dim_n}')
        ...
        self.args.obs_dim = max(self.args.obs_dim_n)  # The dimensions of an agent's observation space
    ...
    def run_episode_mpe(self, evaluate=False):
        ...
        for episode_step in range(self.args.episode_limit):
            if all(dim == self.args.obs_dim_n_raw[0] for dim in self.args.obs_dim_n_raw):
                obs_n = [obs_raw[agent_name] for agent_name in self.agent_names]
            else:
                #zero padding of lower dimension observation
                obs_n = []
                for agent_name in self.agent_names:
                    obs = obs_raw[agent_name]
                    obs = np.concatenate((obs, np.zeros(self.args.obs_dim_n[0] - obs.shape[0])))
                    obs_n.append(obs)
            #print(f'obs_n:{obs_n}, shape of obs_n:{np.array(obs_n).shape}')
            ...
            #wandb.log({'agent_0_reward': r_n_dict['agent_0']}, step=self.total_steps)
            #wandb.log({'agent_1_reward': r_n_dict['agent_1']}, step=self.total_steps)
            #wandb.log({'agent_2_reward': r_n_dict['agent_2']}, step=self.total_steps)
