    ...
    if num_C3 == 0:
        for c1 in C1_i:
            for x2 in range(num_C1 + num_C2):
                if c1 != x2:
                    key = (min(c1, x2), max(c1, x2))
                    comm_dist = pos_dist.get(key, np.linalg.norm(pos_coords[c1] - pos_coords[x2], ord=2))
        ...
        for c2 in C2_i:
            for x2 in range(num_C1 + num_C2):
                if c2 != x2:
                    key = (min(c2, x2), max(c2, x2))
                    comm_dist = pos_dist.get(key, np.linalg.norm(pos_coords[c2] - pos_coords[x2], ord=2))
                    pos_dist[key] = comm_dist
