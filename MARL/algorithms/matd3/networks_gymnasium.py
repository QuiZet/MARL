...
class Actor(nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        #a = (self.max_action - self.min_action) * ((torch.tanh(self.fc3(x)) + 1.0) / 2.0) + self.min_action
...
class Critic_MADDPG(nn.Module):
    def __init__(self, args):
        ...
        print(f'args.obs_dim_n:{args.obs_dim_n}')
        obs = [args.obs_dim_n[item] for item in args.obs_dim_n] 
        acts = [args.action_dim_n[item] for item in args.action_dim_n] 
        print(f'sum:{sum(obs)} {sum(acts)} {sum(obs) + sum(acts)}')
        self.fc1 = nn.Linear(sum(obs) + sum(acts), args.hidden_dim)
        self.fc2 = nn.Linear(args.hidden_dim, args.hidden_dim)
...
class Critic_MATD3(nn.Module):
    def __init__(self, args):
        ...
        self.fc4 = nn.Linear(sum(obs) + sum(acts), args.hidden_dim)
